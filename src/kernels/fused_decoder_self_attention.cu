...
{
    // (Vincent) note: 每个token所属的id, 它的freq值都是固定的, id的上限为max position embedding
...
__global__ void masked_MHA_kernel(T* q,
    ...
    // (Vincent) note: below one line is wrong kv head id access way.
    ...
    if (tid * vec_size < head_size) {
        ...
        // (Vincent) note: will enable below code lines when qkv bias is available
        // if (qkv_bias != nullptr){
    ...
    // (Vincent) note: define dynamic smem type is char type!! not T
    ...
    __syncthreads();
    // (Vincent) note: convert some constant scalar to vector type for vectorizedly computation
    float zero = 0.0f;
    Vec_t zero_f4 = scalar_cast_vec<Vec_t, T>(zero);
    float4 scale_f4 = scalar_cast_vec<float4, float>(scale);

    for(int iter = 0; iter < step; iter++) {
        // (Vincent) note: every iter,  q and k vector's shape = [1, head size]
        // reuse k cache
        ...
        // (Vincent) note: when final step, update k cache and fetch k from input k vec, rather than kv cache
...
__global__ void masked_MHA_kernel(half* q,
                    ...
                    int   rotary_embedding_dim,
                    float rotary_embedding_base){// rsqrt(dh)
        // (Vincent) note: to sync with newest fp32 mha
//     int tid = threadIdx.x;
//     //int bid = blockIdx.x;
...
void launchDecoderMaskedMHA(TensorWrapper<T>* qkv_buf,
    ...
    // (Vincent)note: we should carefully get shape from tensorwrapper, DON'T be GOT wrong value
