...
#include <cuda_fp16.h>
//(Vincent)note: below 5 overloaded function can convert different scalar type data to specified vector type data.
...
{
    //(Vincent)note: __half2half2 cant be parsed by my nvcc compiler, so I give it up
...
//(Vincent)note: temply dont know which LLM use two continuous elements do RoPE
struct TwoFloat2{
    float2 x;
    float2 y;
};
